...
#include "ThreadRegions/include/Graphs/MayHappenInParallel.h"
...
bool LLVMDependenceGraph::getInstructionSites(const std::set<llvm::Instruction *> slicingInstruction,
                                       std::set<LLVMNode *> *callsites)
{
    for (auto ins :slicingInstruction ){
        auto node = findInstruction(ins, constructedFunctions);
        if (node == nullptr) {
            errs() << "Could not find correstponding node\n";
            errs() << *ins << "\n";
        }
...
LLVMNode *findInstruction(llvm::Instruction * instruction, const std::map<llvm::Value *, LLVMDependenceGraph *> & constructedFunctions) {
    ...
    // for (auto i : constructedFunctions) {
    //     errs() << *(i.first)<<"n";
    // }
    // errs() << "====\n";
